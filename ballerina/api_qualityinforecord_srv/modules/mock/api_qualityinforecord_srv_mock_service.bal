...
service /sap/opu/odata/sap/API_QUALITYINFORECORD_SRV on ep0 {
    ...
    # http:Response (Error)
    resource function get QualityFirstArticleInspection(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, QualityFirstArticleInspectionOrderByOptions? \$orderby, QualityFirstArticleInspectionSelectOptions? \$select, QualityFirstArticleInspectionExpandOptions? \$expand) returns CollectionOfQualityFirstArticleInspectionWrapper|http:Response {
        return {
            d: {
                results: [
                    {
    ...
    # http:Response (Error)
    resource function post QualityFirstArticleInspection(@http:Payload CreateQualityFirstArticleInspection payload) returns QualityFirstArticleInspectionWrapper|http:Response {
        http:Response res = new;
        res.statusCode = 500;
        return res;
    }
