...
service /sap/opu/odata/sap/API_INSPECTIONPLAN_SRV on ep0 {

    resource function head .() returns http:Response {
        http:Response res = new;
        res.statusCode = 200;
        res.setHeader("X-CSRF-TOKEN", "Inspection Master Data Integration");
        return res;
    }
    ...
    # http:Response (Error)
    resource function get A_InspectionPlan(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, A_InspectionPlanOrderByOptions? \$orderby, A_InspectionPlanSelectOptions? \$select) returns CollectionOfA_InspectionPlanWrapper|http:Response {
        return {
            "d": {
                "InspectionPlanGroup": "IP_API_1",
                "InspectionPlan": "1",
                "InspectionPlanInternalVersion": "1",
                "BillOfOperationsDesc": "Test API",
                "Plant": "1010",
    ...
    # http:Response (Error)
    resource function post A_InspectionPlan(@http:Payload CreateA_InspectionPlan payload) returns A_InspectionPlanWrapper|http:Response {
        http:Response res = new;
        res.statusCode = 500;
        return res;
    }
